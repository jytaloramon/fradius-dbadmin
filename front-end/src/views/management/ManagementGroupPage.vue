      ...
      <form class="text-sm">
        <div class="mb-4">
          <div class="font-semibold">
            ...
            <simple-tag v-if="!oldGroup.data" text="Novo" class="float-right text-xs text-slate-50 bg-green-600" />
            <simple-tag v-else-if="openGroupNameAndOldIsDifferent()" :text="`Alterado: ${oldGroup.data.name}`"
              class="float-right text-xs text-slate-50 bg-red-500" />
          </div>

          <div class="mt-1 text-sm">Configure permissões/usuários deste grupo.</div>
        </div>
        ...
        <div v-for="(ruleGroup, idx) in ruleGroups" :key="idx" class="pt-4 border-t border-dashed border-gray-300">
          <div class="">
            ...
            <span class="mt-0.5 float-right">
              <checkbox-with-square-icon :is-checked-input="adminGroupOpen.ruleGroupsAllSelected[idx]"
                @change="e => inputRuleGroupChanged(ruleGroup.rules, e.target?.checked)" />
            </span>
          </div>
          ...
          <ul class="grid grid-cols-4 py-3 px-4">
            <li v-for="rule in ruleGroup.rules" :key="rule.id" class="py-2">
              <span class="mt-0.5">
                <checkbox-with-square-icon :is-checked-input="adminGroupOpen.actualRule.has(rule.id)"
                  @changed="e => inputRuleChanged(rule.id, e.target?.checked)" />
              </span>
        ...
        <div class="pt-4 mt-4 border-t border-dashed border-gray-300">
          <div class="text-center font-semibold">Lista de Usuários</div>
            ...
            <app-table :pagination="{ actual: 1, max: 2 }">
              ...
              <template v-slot:body>
                <tr v-for="(group, idx) in tableData.getItems()" :key="idx"
                  ...
                  <td class="p-3">@{{ group.name }}</td>
                  <td class="p-3">
                    <simple-tag v-if="group.qtUsers !== 50" text="Ativo" class="text-xs text-slate-50 bg-green-600" />
                    <simple-tag v-else text="Inativo" class="text-xs text-slate-50 bg-red-500" />
                  </td>
                </tr>
              </template>
            </app-table>
...
import SimpleTag from '@/components/text/SimpleTag.vue';
...
export default defineComponent({
  ...
  components: { AppBox, AppTable, ButtonWithIcon, CheckboxWithSquareIcon, ButtonSquare, SimpleTag, TableHeadItemWithSort },
  ...
  data() {
    return {
      ...
      oldGroup: {
        data: undefined as unknown as IRuleGroup,
        isDifferent: false,
      },
      form: {
        field: {
  ...
  methods: {
    openGroupNameAndOldIsDifferent(): boolean {
